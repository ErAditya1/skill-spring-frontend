...
export default function CustomVideoPlayer({
  ...
  if (playedSecondsRef.current < 5) return

  try {
    await api.post('/v1/courses/course/update-last-watched', {
      courseId: course_id,
      videoId: videoId,
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    videoQue.map((v, index) => {
      if (v === videoId) {
        setPreviousVideo(
          index === 0 ? videoQue[videoQue.length - 1] : videoQue[index - 1],
        );
        setNextVideo(
          index === videoQue.length - index ? videoQue[0] : videoQue[index + 1],
        );
      }
    });
  }, [videoId]);
  ...
  const handleProgress = (state: { loaded: number; played: number; playedSeconds: number }) => {
    if (!seeking) {
  ...
  const handleEnded = () => {
    ...
    if (nextVideo) {
      ...
      router.push(`/student/courses/${course_id}/${nextVideo}`);
    } else {
      setStart(false);
      setPlaying(false);
      setLoop(true);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`${!ready && "hidden"} absolute top-0 w-full h-full select-none`}
          onMouseMove={showControls}
          onClick={showControls}
        >
          <div
            className={`w-full h-full ${!controls && "hidden duration-1000"}`}
          >
            <div className="w-full h-full flex justify-evenly  items-center">
              <button
                className="text-xl mx-2 cursor-pointer rounded-full p-1 bg-white dark:bg-gray-800  bg-opacity-30 "
            ...
            <div className="w-full h-17 absolute bg-card" style={{ bottom: 0 }}>
              ...
              <div className="w-full h-8 flex items-center justify-between px-2  relative">
                  ...
                  <div className="relative mx-1 flex flex-row items-center gap-1">
                    {prevVideo && (
                      <button
                        ...
                        onClick={() =>
                          router.push(
                            `/student/courses/${course_id}/${prevVideo}`,
                          )
                        }
                      >
                    ...
                    {nextVideo && (
                      <button className="" onClick={handleEnded}>
                ...
                <div className="right-4 flex  gap-3 justify-center items-center">
                  {isSetting && (
                    <div
                      className="w-48 border bg-background absolute bottom-12 right-0 p-2 mr-4 rounded-lg"
                      ref={dropdownRef}
                    >
                      <div className="flex justify-between items-center w-full relative py-2 ">
                        ...
                        <button
                          ...
                          className="text-2xl right-2 absolute "
                        >
                          <MdOutlineClose
                            size={20}
                      ...
                      {isSpeed || isQuality ? (
                        <div className="flex gap-2 flex-col">
                          {isSpeed &&
                            [0.25, 0.5, 1, 1.5, 1.75, 2].map((item, index) => (
                              <div>
                                <button
                                  key={index}
                        ...
                        <div>
                          <div
                            className="flex justify-between items-center w-full relative py-1"
