...
function QuizList({ items, onRender, onEdit }: quizListProps) {
    ...
    const onDragEnd = (result: DropResult) => {
        ...
        const updatedquizs = items.slice(startIndex, endIndex + 1)

        // setquizs(items)


        const bulkItems = updatedquizs.map((quiz) => ({
            id: quiz._id,
                        ...
                        {quizes?.map((quiz, index) => (
                            ...
                            >
                                {(provided) => (
                                    ...
                                    >
                                        <div
                                            className={`px-2 py-2 border-r mr-2 ${quiz.isPublished ? '' : ''}`}
                                            {...provided.dragHandleProps}
                                        >
                                            <Grid className='h-5 w-5' />
                                            {/* <h3 className='line-clamp-1'>{quiz.title}</h3> */}
                                            {/* <button onClick={() => onEdit(quiz._id)}>Edit</button> */}
                                        </div>
                                        <span className='line-clamp-2'> {quiz?.title}</span>
                                        <div className='ml-auto pr-2 flex items-center gap-x-2'>

                                            <Badge>{quiz?.isFree ? "Free" : "Paid"}</Badge>
                                            <Badge className={`bg-slate-500 ${quiz?.isPublished && 'bg-sky-700'}`}>
