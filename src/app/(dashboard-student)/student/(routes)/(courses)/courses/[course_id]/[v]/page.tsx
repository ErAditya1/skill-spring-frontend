...
function Page() {
  ...
  useEffect(() => {
    if (v) {
      api
        .get(`/v1/videos/video/get-video-data/${v}`)
        .then((res) => {
          if (res.status === 200) {
  ...
  return (
    <div className="">
      <div className="md:mb-2 w-full gap-4  flex flex-col lg:flex-row ">
        <Card
          className="grow dark:bg-background p-0 text-card-foreground rounded-lg "
          style={{ padding: 0 }}
        >
          <div className=" aspect-video col-span-8 rounded-xl  max-h-dvh max-w-screen">
            {v && !loadingVideo ? (
              <CustomVideoPlayer
                videoId={v}
                thumbnailUrl={playingVideoData?.thumbnail?.secure_url}
                title={playingVideoData?.title}
                videoQue={videos.map((item) => item.videoId)}
              />
            ) : (
          ...
          <div className="p-4 ">
            {loading ? (
              ...
              <h1 className="text-sm  font-semibold line-clamp-1 lg:text-md">
            ...
            <div className="flex flex-row flex-wrap items-center">
              <React.Fragment>
                {loading ? (
                  <Skeleton className="h-10 w-10 mr-1  rounded-full" />
                ) : (
                  <AvatarLayout
                    className="h-10 w-10 mr-1 text-xl"
                    src={playingVideoData?.author?.avatar?.url}
                    name={playingVideoData?.author.name}
                    username={playingVideoData?.author?.username}
                  />
                )}
                ...
                {loading ? (
                  ...
                  <div className="card-content mx-2">
                    <p className="line-clamp-1 text-sm lg:text-md">
                      {playingVideoData?.author?.name}
                    </p>
                    <p className="line-clamp-1 text-xs lg:text-md">
                      @{playingVideoData?.author?.username}
                    </p>
                  </div>
                )}
                {loading ? (
                    ...
                    {playingVideoData?.isAuthor ? (
                      ...
                      <div className="m-2 ">
                        <FollowButton
                          ...
                          _id={playingVideoData?.author?._id}
                          isFollowing={playingVideoData?.isFollowing}
                          count={playingVideoData?.followersCount}
                        />
                      </div>
                    )}
                  </div>
                ...
                <React.Fragment>
                  <div className="m-2 rounded  p-1">
                    <LikeButton
                      ...
                      liked={playingVideoData?.isLiked}
                      likeCnt={playingVideoData?.likeCount}
                      type="video"
                  ...
                  {playingVideoData?.isFree && (
                    <div className="m-2 ">
                      <ShareButton
                        className="rounded-full text-xl"
                        textToShare={playingVideoData?.title}
                      />
                    </div>
                  )}
                ...
                >
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: playingVideoData?.description,
                    }}
                  />
                </Typography>
              )}
        ...
        <div className="max-h-dvh w-full flex flex-col lg:flex-none lg:max-h-dvh  lg:min-w-[300px] lg:w-[35%] rounded-xl border overflow-auto relative ">
          <div className="p-2   sticky top-0  rounded-t-xl ">
            <h1 className="text-2xl font-bold ">
              Comments {playingVideoData?.commentCount}:
            </h1>
            <CommentCard
              _Id={playingVideoData?._id}
          ...
          <div className=" w-full h-full gap-4"></div>
        </div>
      </div>
    </div>
  );
